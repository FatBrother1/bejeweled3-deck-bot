...
做法很直白：从游戏内存里把棋盘直接读出来（只读，从不写入），算一步，
动鼠标。游戏文件一个字节不改。另外做了个 Decky 插件，这样在游戏模式里
也能开关，不用切到桌面模式开终端。

代码全部由 AI 编写（deepseek-v4.1-flash 为主，后续迭代用了
glm-5.3-flash），我一行没写。
...
版本号只有**一个来源**：Decky 插件的 `plugin.json`。
GitHub Release 的 tag 和它保持一致，不再各走一套。

**发布过的版本号不再动它的资产。** 有改动就递增（0.2.1 → 0.2.2 → …）。
之前我犯过一次错：新内容直接覆盖了 0.2.1 的资产，
结果那个 tag 指向的提交和它挂的包不是一回事，
已经下载过的人拿到的东西跟仓库对不上。已恢复，并改成递增。

之前这里是乱的 —— Release 打到 1.3.0，插件却一直是 0.2.1，
而且 `plugin.json` 里压根没有 `version` 字段（Decky 读的是它，
所以插件显示的是「无版本」）。现在补齐并对齐。

## 支持的游戏模式

```bash
python3 bot_v6.py --mode normal   # 普通模式（默认）
python3 bot_v6.py --mode poker    # 牌局模式
```

牌局模式的规则和普通模式完全不同：每消除一次得到一张该花色的牌，
集齐 5 张凑成牌型才给分。同花 50000 分，而且永远不会带来骷髅标记，
所以 bot 死盯一个花色凑同花。

实测 70 步分数从 18000 涨到 103900，分/秒 757.3（普通模式是 144.5）。
机制细节见 [`bot/牌局模式说明.md`](bot/牌局模式说明.md)。

## 它怎么工作的

```
读内存（只读）→ 拿到棋盘 → 算一步 → 模拟鼠标
PipeWire 抓屏 → 只用来判断结算画面和识别模式，不参与认棋盘
```

拿棋盘有两种方式，默认自动选：

**视觉** —— 截屏识别颜色，最早的做法，现在作为后备。
...
静止怎么判：现在是内存轮询——棋盘指纹和分数都不动了就算静止，分数一动
也算运动信号。早期版本是像素差判静止，后来重构时被同名的内存轮询版覆盖了
（这个坑记在开发日志里）。过场动画期间内存数据是冻结的，这时 bot 靠结算画面
判据兜底，不会乱出手。
